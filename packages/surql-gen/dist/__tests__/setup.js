...
export function expectSurrealQLContains(sql, ...patterns) {
    for (const pattern of patterns) {
        if (!sql.includes(pattern)) {
            throw new Error(`Expected SurrealQL to contain "${pattern}" but it didn't.\nActual SQL:\n${sql}`);
        }
    }
}
export function expectSurrealQLNotContains(sql, ...patterns) {
    for (const pattern of patterns) {
        if (sql.includes(pattern)) {
            throw new Error(`Expected SurrealQL to NOT contain "${pattern}" but it did.\nActual SQL:\n${sql}`);
        }
    }
}
