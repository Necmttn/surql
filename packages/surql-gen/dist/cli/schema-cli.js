...
const _generateCommand = async (options) => {
...
const compareSchemaCommand = new Command()
    ...
    const program = Effect.gen(function* (_) {
        ...
        if (diff.hasChanges) {
            ...
            const migration = MigrationGenerator.generateMigration(diff, `migration_${Date.now()}`, "Auto-generated migration from schema comparison");
...
const validateSchemaCommand = new Command()
    ...
    const program = Effect.gen(function* (_) {
        ...
        const schema = SurrealSchema
            ...
            .addTable(SurrealTable.create("profile", [
        ...
        try {
            ...
            yield* Console.log(`   Tables: ${schema.getTables().length}`);
            yield* Console.log(`   Valid references: ${schema.getTables().reduce((count, table) => count + table.getFields().filter(f => f.getReferences()).length, 0)}`);
        }
        catch (error) {
