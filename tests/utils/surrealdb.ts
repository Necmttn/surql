...
export async function createTestSchema(content: string, filename = "test_schema.surql"): Promise<string> {
...
export class SurrealDBInstance {
  ...
  async loadSchema(schemaPath: string, namespace = "test", database = "testdb"): Promise<void> {
    console.log(`Loading schema from ${schemaPath} into namespace=${namespace}, database=${database}...`);
...
`;
    ...
    const tempSchemaPath = await createTestSchema(schemaContent, `${namespace}_${database}_init.surql`);
...
export function createTestConfig(url: string, namespace = "test", database = "testdb"): Config {
  return {
    output: {
      path: "./generated",
      filename: "schema",
