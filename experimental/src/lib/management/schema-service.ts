...
import { SurrealTyped } from "../../../../../api/lib/db.ts";
import { SchemaComparator, SchemaDiff } from "../comparison/diff.ts";
import { type Migration, MigrationGenerator } from "../migration/generator.ts";
...
export class SchemaManagementService extends Effect.Service<SchemaManagementService>()(
  "SchemaManagementService",
  {
    effect: Effect.gen(function* (_) {
      ...
      const calculateChecksum = (schema: SurrealSchema): string => {
      ...
      const applySchema = (schema: SurrealSchema) =>
      ...
      const applyMigration = (migration: Migration) =>
        Effect.gen(function* (_) {
          ...
          yield* MigrationGenerator.validateMigration(migration).pipe(
          ...
          try {
            ...
            const checksum = calculateChecksum(migrationFile as any); // Quick hash of migration content
      ...
      // Rollback migration
      const rollbackMigration = (migrationName: string) =>
        Effect.gen(function* (_) {
      ...
      const migrateToSchema = (targetSchema: SurrealSchema) =>
        Effect.gen(function* (_) {
          ...
          yield* Effect.logInfo(
            `Migrating from ${currentMetadata.version} to ${targetSchema.getVersion()}`
          );
      ...
      const validateDatabaseSchema = (expectedSchema: SurrealSchema) =>
        Effect.gen(function* (_) {
