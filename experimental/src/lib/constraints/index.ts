 ...
 */
export const strongPassword = Schema.pattern(
  /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]+$/,
  {
 ...
 */
export const uuidPattern = Schema.pattern(
 ...
 */
export const minLength = (min: number) => Schema.minLength(min, {
  message: (issue) => `String too short: ${issue.actual} characters (min ${min})`
});

export const maxLength = (max: number) => Schema.maxLength(max, {
  message: (issue) => `String too long: ${issue.actual} characters (max ${max})`
});

export const exactLength = (length: number) => Schema.length(length, {
 ...
 */
export const minNumber = (min: number) => Schema.greaterThanOrEqualTo(min, {
  message: (issue) => `Number too small: ${issue.actual} (min ${min})`
});

export const maxNumber = (max: number) => Schema.lessThanOrEqualTo(max, {
  message: (issue) => `Number too large: ${issue.actual} (max ${max})`
});

export const numberRange = (min: number, max: number) => Schema.between(min, max, {
 ...
 */
export const customStringConstraint = (
  pattern: RegExp,
  minLen: number,
  maxLen: number,
  ...
  nonEmptyString,
  Schema.pattern(pattern, { message: () => patternMessage }),
  minLength(minLen),
  maxLength(maxLen)
);
 ...
 */
export const customNumberConstraint = (
  min: number,
  max: number,
 ...
 */
export const enumConstraint = <T extends readonly [string, ...string[]]>(...values: T) =>
