...
Deno.test("parseType handles various SurrealQL types correctly", () => {
  // Test basic types
  assertEquals(parseType("string"), { baseType: "string", isOption: false, reference: undefined });
  assertEquals(parseType("number"), { baseType: "number", isOption: false, reference: undefined });
  assertEquals(parseType("bool"), { baseType: "bool", isOption: false, reference: undefined });

  // Test option types
  assertEquals(parseType("option<string>"), { baseType: "string", isOption: true, reference: undefined });
  assertEquals(parseType("option<number>"), { baseType: "number", isOption: true, reference: undefined });
  ...
  // Test array types
  assertEquals(parseType("array"), { baseType: "array", isOption: false, reference: undefined });
  assertEquals(parseType("option<array>"), { baseType: "array", isOption: true, reference: undefined });
...
Deno.test("generateTypeBoxSchemas generates correct TypeBox schema", () => {
  ...
  const result = generateTypeBoxSchemas(tables);

  // Check for key elements in the output
  assertEquals(result.includes("export const TelegramUserType = Type.Object({"), true);
  assertEquals(result.includes("export const TelegramMessageType = Type.Recursive(This =>"), true);
  assertEquals(result.includes("$id: 'telegram_user'"), true);
  assertEquals(result.includes("$id: 'telegram_message'"), true);
  assertEquals(result.includes("messages: Type.Array(RecordIdType('telegram_message'))"), true);
  assertEquals(result.includes("user: RecordIdType('telegram_user')"), true);

  // Fixed assertion to match actual implementation which uses This for recursive references
  assertEquals(result.includes("reply_to_message_id: Type.Optional(This)"), true);

  assertEquals(result.includes("const Module = Type.Module({"), true);
  assertEquals(result.includes("telegram_user: TelegramUserType"), true);
  assertEquals(result.includes("const TelegramUser = Module.Import('telegram_user')"), true);
});
