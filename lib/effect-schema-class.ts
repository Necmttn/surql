...
export const flexibleRecordId = <T extends string>(tableName: T) => Schema.Union(
  Schema.instanceOf(RecordId).pipe(
    Schema.filter((id) => id.tb === tableName, {
      message: () => \`Expected RecordId for table \${tableName}\`,
    })
  ),
  Schema.instanceOf(StringRecordId).pipe(
    Schema.filter((id) => id.toString().startsWith(\`\${tableName}:\`), {
      message: () => \`Expected StringRecordId for table \${tableName}\`,
    })
  ),
	recordIdSchema(tableName),
	stringRecordIdSchema(tableName),
