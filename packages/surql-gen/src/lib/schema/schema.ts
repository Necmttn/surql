...
// Use the Schema.Class implementations directly
type SchemaIndexType = SurrealIndex;
type SchemaEventType = SurrealEvent;
 ...
 */
export class SurrealSchema extends Schema.Class<SurrealSchema>("SurrealSchema")({
  ...
  static fromTables(name: string, tables: readonly SurrealTable[], version = "1.0.0"): SurrealSchema {
    return this.parse({
      name,
      version,
      tables: [...tables]
  ...
  toSurrealQL(): string {
    ...
    for (const index of this.getIndexes()) {
      ...
      if (index.fulltext) sql += " SEARCH ANALYZER ascii BM25";
      sql += ";\n";
    }
    ...
    for (const event of this.getEvents()) {
      ...
      if (event.condition) sql += ` WHERE ${event.condition}`;
      sql += ` THEN ${event.then};\n`;
    }
  ...
  static fromSurrealQL(sql: string): SurrealSchema {
    ...
    for (const line of lines) {
      if (line.startsWith('-- Schema:')) {
        ...
        if (match) {
          name = match[1];
          if (match[2]) version = match[2].trim();
        }
