...
import { exportSchemaFromDB } from "../lib/db.ts";
...
`;
	...
	const fileNormalSchemafull = join(
		fixturesDir,
		"schema_normal_schemafull.surql",
	);
	...
	await t.step("should parse different schema formats consistently", () => {
		...
		assertEquals(
			tableDefsAnySchemaless.length,
			1,
			"Should parse one table from ANY SCHEMALESS",
		);
		assertEquals(
			tableDefsNormalSchemafull.length,
			1,
			"Should parse one table from NORMAL SCHEMAFULL",
		);
		assertEquals(
			tableDefsWithOverwrite.length,
			1,
			"Should parse one table with OVERWRITE",
		);

		// All should have the same fields
		assertEquals(
			tableDefsAnySchemaless[0].fields.length,
			3,
			"Should parse 3 fields from ANY SCHEMALESS",
		);
		assertEquals(
			tableDefsNormalSchemafull[0].fields.length,
			3,
			"Should parse 3 fields from NORMAL SCHEMAFULL",
		);
		assertEquals(
			tableDefsWithOverwrite[0].fields.length,
			3,
			"Should parse 3 fields from schema with OVERWRITE",
		);
		...
		compareFields(
			tableDefsAnySchemaless[0].fields,
			tableDefsNormalSchemafull[0].fields,
		);
		compareFields(
			tableDefsAnySchemaless[0].fields,
			tableDefsWithOverwrite[0].fields,
		);
	});

	await t.step(
		"should handle OVERWRITE flag in exportSchemaFromDB",
		async () => {
			// Apply a base schema
			await dbInstance.loadSchema(
				fileNormalSchemafull,
				"test",
				"schema_format_test",
			);
			await delay(500);
