...
export const OutputConfigSchema = Type.Object(
	{
		...
		extension: Type.Union(
			[Type.Literal("ts"), Type.Literal("js"), Type.Literal("d.ts")],
			{ default: "ts" },
		),
	},
	{
 ...
 */
export const SchemaSystemSchema = Type.Union([Type.Literal("effect")], {
	default: "effect",
});
...
export const ImportsConfigSchema = Type.Object(
	{
		style: Type.Union(
			[Type.Literal("esm"), Type.Literal("commonjs"), Type.Literal("deno")],
			{ default: "esm" },
		),
...
export async function loadConfigFromFile(configPath: string): Promise<Config> {
	try {
		...
		console.error(
			`Error loading config from ${configPath}: ${error instanceof Error ? error.message : String(error)}`,
		);
...
async function loadTypeScriptConfig(configPath: string): Promise<Config> {
	try {
		...
		console.error(
			`Error loading TypeScript config from ${configPath}: ${error instanceof Error ? error.message : String(error)}`,
		);
		return DEFAULT_CONFIG as Config;
	}
}
...
export function getOutputPath(config: Config): string {
	return join(
		config.output.path,
		`${config.output.filename}.${config.output.extension}`,
	);
}
